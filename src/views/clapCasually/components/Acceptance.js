...
const { required, maxLength, requiredMessage } = rules;
...
export default memo(() => {
    ...
    const rules = useCallback(() => {
        const rule = {
            reply: [required, maxLength(300)],
            image: [requiredMessage("图片:该项")],
        };
        return rule;
    }, []);
        ...
        <>
                ...
                type === 0 && <div className={styles.acceptanceContainer}>
                    <Form ref={formRef} model={form} rules={rules()}>
                        <FormItem required prop="reply" label="处理回复" className={styles.formItem}>
                            <TextArea placeholder={"请输入回复"} maxLength={300} value={form && form.reply || ""} onChange={handleChange("reply")} />
                            <div className={styles.textNum}>{textNum >= 300 ? 300 : textNum}/300</div>
                        </FormItem>
                        <FormItem label="上传图片" subTitle="最多上传6张" required>
                            <ImageUpload
                                maxLength={6}
            ...
            }
            {
                type === 1 && <div className={styles.returnList}>
                    <img alt="" className={styles.image} src={require('../../../assets/images/newImg/success.png')} />
                    <div className={styles.tips}>当前事件已办结！</div>
                    <button onClick={() => pushUrl({ pathname: "/clapCasually" })}>返回列表</button>
                </div>
            }
        </>
    )
