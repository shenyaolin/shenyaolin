...
const stateList = [
    { label: '待回复', type: 0, status: 0 },
    { label: '已回复', type: 1, status: 1 },
]
const timeList = [
    { label: '由近到远', type: 1 },
    { label: '由远到近', type: 2 },
...
export default memo(({ show, handleScreen, isShow = true }) => {
    ...
    const handleRouter = useCallback((type1) => {
        const { status, sortEnum, type } = search
        if (type1 === "search") {
            handleScreen({ status, sortEnum, type })
        } else if (type1 === "close") {
            handleScreen("close")
        } else {
            setSearch({})
                    ...
                    <div className={styles.content}>
                        {
                            stateList.map((item, index) => {
                                return (
                                    <div key={index} className={classnames(styles.options, search.status == item.type && styles.choose)} onClick={() => eventClick(item)}> {item.label}</div>
                                )
                            })
                        }
                    </div>
