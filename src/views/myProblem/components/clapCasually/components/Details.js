      ...
      <>
        {
          newList && newList.length == 1 ? <li className={styles.progress}>您上报的事件正在处理中~</li> :
            newList.map((item, index) => {
              return (
                <li className={classnames(index === 0 && styles.last_progress, item.image && styles.leavingImgs)} key={index}>
                  <div className={styles.progress_content}>
                    <div className={classnames(styles.progress_title, styles.active)}>
                      ...
                      {
                        index === 0 && <div className={styles.message}>
    ...
    <>
      {
        show && <div className={styles.details}>
            ...
            <div className={classnames(styles.images, styles[imgClass(snapshot.eventImage)])}>
              {
                snapshot.eventImage && snapshot.eventImage.split(',').map((item, index) => {
                  return <img src={imageSrc(item)} key={index} />
                })
              }
            </div>
            <div className={styles.villageName}>{snapshot.villageName}</div>
          </div>
          <div className={classnames(styles.steps, snapshot?.evaluateType && styles.steps1)}>
          ...
          {
            snapshot.evaluateType == null ? " " : <div className={styles.evaluate}>
          ...
          {
            userType === 2 && snapshot.list.length > 0 && snapshot.list[snapshot.list.length - 1].schedule === 3 && <div className={styles.acceptanceBtn}><Button onClick={() => wxPushUrl({
