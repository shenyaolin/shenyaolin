...
export default memo(() => {
    ...
    return (
        <>
            {
                show && <div className={styles.detailContainer}>
                    ...
                    <div className={styles.imgs}>
                        {
                            results.image && results.image.split(',').map((img, index) => <img className={classnames(styles[handleImgSize(results.image)])} key={index} alt="" src={imageSrc(img)} />)
                        }
                    </div>
                                ...
                                results.adviceReply ?
                                    <div>
                                        ...
                                        <div className={styles.replyContentCenter}>
                                            <div>答复意见</div>
                                            <div className={styles.adoptState}>{results.adoptionStatus === 1 ? "已采纳" : '未采纳'}</div>
